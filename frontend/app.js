...
async function abandonActiveWorkout(workoutId) {
    ...
    if (confirm('Êtes-vous sûr de vouloir abandonner cette séance ?')) {
        ...
        try {
            console.log('🚨 [DEBUG] Nettoyage complet de toutes les séances abandonnées...');
            
            // ✅ SOLUTION : Supprimer TOUTES les séances abandonnées d'un coup
            await apiDelete(`/api/users/${currentUser.id}/workouts/abandoned/cleanup`);
            showToast('Toutes les séances abandonnées supprimées', 'info');
            
        } catch (error) {
            console.error('Erreur nettoyage global:', error);
            // Fallback : supprimer au moins celle-ci
            try {
                ...
                showToast('Séance abandonnée supprimée', 'info');
            } catch (fallbackError) {
...
async function skipExercise(exerciseId, reason) {
    ...
    if (window.currentWorkoutSession?.type === 'ai') {
        ...
        if (nextIndex < window.currentWorkoutSession.exercises.length) {
            await window.selectExerciseFromAISession(
...
async function skipExerciseFromSession(exerciseIndex) {
...
// Exposer globalement
window.skipExerciseFromSession = skipExerciseFromSession;
...
async function restartSkippedExercise(exerciseId) {
    ...
    await selectSessionExercise(exerciseId);
    ...
    try {
        ...
        if (window.currentWorkoutSession.type === 'ai') {
            ...
            if (currentSet >= baseSets) {
                ...
                const exerciseId = parseInt(currentExercise.exercise_id || currentExercise.id);
                
                // Marquer exercice comme complété pour cette série
                if (window.currentWorkoutSession.sessionExercises && window.currentWorkoutSession.sessionExercises[exerciseId]) {
                    const exerciseState = window.currentWorkoutSession.sessionExercises[exerciseId];
                    exerciseState.isCompleted = true;
                    exerciseState.completedSets = currentSet;
                }
                
                // Trouver l'index de l'exercice courant (avec conversion de type)
                const currentIndex = window.currentWorkoutSession.exercises.findIndex(
                    ex => parseInt(ex.exercise_id || ex.id) === exerciseId
                );
                
                // Validation de l'index trouvé
                if (currentIndex === -1) {
                    console.error(`[AI Modal] Exercice ${exerciseId} non trouvé dans la session AI`);
                    return;
                }
...
function completeRest() {
    ...
    if (currentSet >= window.currentWorkoutSession.totalSets) {
        // ========================================================================
        // CORRECTION MAJEURE: Supprimer duplication - saveFeedbackAndRest() gère déjà les séances AI
        // Cette section ne s'exécute QUE pour les séances libres/programmes
        // ========================================================================
        if (window.currentWorkoutSession.type !== 'ai') {
            // Séances libres/programmes : comportement classique PRÉSERVÉ
            transitionTo(WorkoutStates.COMPLETED);
            showSetCompletionOptions();
        }
        // Note: Les séances AI sont entièrement gérées par saveFeedbackAndRest() et ne passent jamais ici
        
    } else {
        // PRÉSERVER: Logique pour série suivante du même exercice (tous types de séances)
...
async function selectExerciseFromAISession(exerciseId, exerciseIndex) {
    if (!window.currentWorkoutSession || window.currentWorkoutSession.type !== 'ai') {
...
// Exposer globalement
window.selectExerciseFromAISession = selectExerciseFromAISession;
...
window.updateSessionExerciseProgress = updateSessionExerciseProgress;
